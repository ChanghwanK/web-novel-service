...
#### Redission 사용 결정 고민: 포인트 충전 로직에 들어간 Redission의 `try.lock()`의 경우 특정 시간 동안 Lock을 획득하지 못하는데 괜찮을까?
- 괜찮다고 판단
  - 괜찮지 않은 케이스는 Lock이 걸린 후 실행되는 로직이 복잡하여 오래걸리고 Lock 점유를 위한 Race Condition이 높을 때 Lock을 획득하지 못해 Error 발생률이 높아짐
  - 일단, 높은 부하가 발생하지 않음
  - Lock이 잡히는 로직은 단순히 Redis에 동일한 결제 번호가 있는지 조회하고 없다면 결제 번호를 저장하거나 있을 경우 실패처리를 시키는 것이기 때문에 로직이 복잡하지 않음 (수행시간이 짧음)
  - 특정 이유로 일정시간Lock을 획득하지 못하는 쓰레드가 많아져도 결제 요청이 실패로 끝나는 것이기 때문에 이후 재시도하면 되기 때문에 비즈니스상 크게 문제 없다고 판단
...
#### 부하 테스트 결과
![Screen Shot 2023-04-19 at 6 27 53 PM](https://user-images.githubusercontent.com/83544156/233032065-30eba455-48c7-4ce4-8f33-08e1320be6af.png)

- 500명의 유저로 부하테스를 진행한 결과 Lock 얻지 못해 실패하는 케이스는 없었습니다.
![Screen Shot 2023-04-19 at 6 30 04 PM](https://user-images.githubusercontent.com/83544156/233032595-a8352200-4918-4ec6-b0ef-781c8c3128f8.png)

- 500은 없고 전부 400으로 떨어짐
